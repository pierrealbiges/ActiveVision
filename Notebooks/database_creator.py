...
def train_dataset_creator(minibatch_size, offset_std=10, offset_max=25, input_size=N_theta*N_azimuth*N_eccentricity*N_phase, asize=N_azimuth*N_eccentricity, label_size=N_azimuth*N_eccentricity):
    
    t_start = time.time()
    
    # Loading the 28*28 MNIST training dataset (60k examples)
    ...
    # Big variables that'll be saved
    inputs = np.zeros((len(train_loader.dataset), 1, input_size))
    labels = np.zeros((len(train_loader.dataset), 1, label_size))
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        for idx in range(minibatch_size):
            ...
            # Updating variables with processed data
            inputs[batch_idx*idx, 0, :], labels[batch_idx*idx, 0, :] = couples(data[idx, 0, :, :], i_offset, j_offset)
            
        if batch_idx % 100 == 0:
            print('[{}/{}] Time: {:.2f} mn'.format(
                batch_idx*minibatch_size, len(train_loader.dataset), (time.time()-t_start)/60))
    
    # Transforming variables into tensors
    inputs = Variable(torch.FloatTensor(inputs))
    labels = Variable(torch.FloatTensor(labels))
    
    # Saving tensors
    torch.save(inputs, './database/train/inputs.pt')
    torch.save(labels, './database/train/labels.pt')
    print('inputs and labels saved')


def test_dataset_creator(minibatch_size, offset_std=10, offset_max=25, input_size=N_theta*N_azimuth*N_eccentricity*N_phase, asize=N_azimuth*N_eccentricity, label_size=N_azimuth*N_eccentricity):
    ...
    for batch_idx, (data, label) in enumerate(test_loader):
        ...
        if batch_idx % 100 == 0:
            print('[{}/{}] Time: {:.2f} mn'.format(
                batch_idx*minibatch_size, len(test_loader.dataset), (time.time()-t_start)/60))
    
    # Transforming variables into tensors
    inputs = Variable(torch.FloatTensor(inputs))
    labels = Variable(torch.FloatTensor(labels))
    
    torch.save(inputs, './database/test/inputs_test.pt')
    torch.save(labels, './database/test/labels_test.pt')
    print('inputs and labels saved')